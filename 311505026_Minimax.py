...
def minmax(depth, isMaximizingPlayer, alpha, beta, color, hexagon_board):
    if depth == 0 or display_remaining_hexes():
        if color == 'black':
            return calculate_connected_areas('black'), None
        else:
            return calculate_connected_areas('white'), None
    best_move = None
    hexes_by_label = {}
    for pos, info in hexagon_board.items():
        ...
        for selected_label in available_labels.keys():
            ...
            for selected_hex in combinations:
                for pos, hex_info in selected_hex:
                    hex_info['selected'] = True
                    hex_info['booked'] = True
                    hex_info['owner'] = color
                if color == 'black':
                    color = 'white'
                else:
                    color = 'black'
                value, _ = minmax(depth-1, False, alpha, beta, color, hexagon_board)
                if (value > bestVal):
                    best_move = selected_hex
                bestVal = max(bestVal, value)
                alpha = max(alpha, bestVal)
                for pos, hex_info in selected_hex:
                    hex_info['selected'] = False
                    hex_info['booked'] = None
                    hex_info['owner'] = None
                if (beta <= alpha):
                    break
...
def select_hexes_by_AI(hexes_by_label, current_round):
    ...
    if current_round == 1:
        ...
        if available_labels:
            if (total < 100):
                new_board = hexagon_board
                alpha = float('-inf')
                beta = float('inf')
                value, hex = minmax(depth=3, isMaximizingPlayer=True, alpha=alpha, beta=beta, color=current_turn, hexagon_board=new_board)
                print("value = ",value)
                selected_hexes = hex
